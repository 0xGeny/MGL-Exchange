...
class UserService {
    ...
    static async verifyEmail(email, locale) {
        ...
        const resp = await emailService.deliverEmail(email, subject, body)

        if (!resp.ok) {
            return {response: false, message: "An error was caused during the Email Verification Code sending.", data:null}
        }
